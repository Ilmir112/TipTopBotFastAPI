...
async def find_service_by_id(service_id: int, user: Users = Depends(get_current_user)):
...
async def update_service_data(
    ...
    try:
        result = await ServiceDAO.update(
            {"service_id": service_id}, service_name=service_name, time_work=time_work
        )
        return result
    except SQLAlchemyError as db_err:
...
async def add_service_data(
    service_data: SService, user: Users = Depends(get_current_user)
):
    try:
        service = await ServiceDAO.find_one_or_none(
        ...
        result = await ServiceDAO.add(
            service_name=service_data.service_name, time_work=service_data.time_work
        )
        if result:
            return result
    except SQLAlchemyError as db_err:
        msg = f"Database Exception Brigade {db_err}"
        logger.error(
            msg, extra={"service_data": service_data.service_name}, exc_info=True
        )
    except Exception as e:
        msg = f"Unexpected error: {str(e)}"
        logger.error(
